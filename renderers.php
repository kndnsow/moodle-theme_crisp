...
class theme_crisp_core_blog_renderer extends core_blog_renderer {
  public function render_blog_entry(blog_entry $entry) {
    ...
    $attachedfiles = $DB->get_records_sql('select * from {files} mf where mf.component="blog" and mf.mimetype IS NOT NULL AND mf.filearea="attachment"
                                           AND mf.filename IN ("'.implode("\",\"",$fileName).'")');
